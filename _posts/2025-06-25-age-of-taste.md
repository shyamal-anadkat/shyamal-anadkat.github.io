...
**My simple thesis is that as we push compute toward its physical limits in the domains where it matters, the work humans do will migrate to the handful of domains where raw floating-point operations per second ([FLOPs](https://en.wikipedia.org/wiki/Floating_point_operations_per_second)) confer little or no advantage. work will reorient around what cannot be sped up by more compute.**

we've been through versions of this. each major tech wave has been a collapse in the **"cost of action."** steam freed us from animal muscle. electrification let us run factories all night. the microprocessor made logic essentially free; software ate the world. GPUs and now specialized AI accelerators took things like rendering, simulation, and gradient descent - once impractically slow - and made them commodities. each time, it wasn't just that we made existing tasks cheaper. We re‑priced entire labor markets. The draftsperson became a CAD operator. The typesetter became a UX designer. and so on. 

[Moore's Law](https://en.wikipedia.org/wiki/Moore%27s_law) - the empirical observation that we could double the number of transistors on a chip every couple of years – is bumping up against atomic limits and thermodynamic floors. But the relevant law for AI isn't just Moore's; it's Wright's. [Wright's Law](https://www.ark-invest.com/wrights-law) says that for every cumulative doubling of units produced, costs fall by a constant percentage. In practice, as we learn to scale these systems, the cost per useful unit of intelligence is on a learning curve that hasn't yet shown signs of bending.
